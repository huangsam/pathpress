...
/**
 * Geocoding utility using OpenStreetMap's Nominatim API to resolve location names
 * (including typos and misspellings like "San Digo" -> "San Diego, CA") into coordinates and clean display names.
 */
object Geocoder {
    ...
    /**
     * Resolves a location string (which may be "lat,lng" coordinates or a city/landmark name with potential typos).
     *
     * @param location Query location string (e.g. "San Jose", "37.33,-121.88", or misspelled "San Digo")
     * @return GeocodedLocation representing the resolved latitude, longitude, and corrected display name
     */
    ...
    private fun queryNominatim(queryString: String): GeocodedLocation? {
        try {
            ...
            val uri = URI.create("https://nominatim.openstreetmap.org/search?q=$encodedQuery&format=json&limit=1&bounded=1&viewbox=-124.4,42.1,-114.1,32.5")
            ...
            if (response.statusCode() == 200 && response.body().isNotBlank()) {
                ...
                if (jsonNodes.isNotEmpty()) {
                    ...
                    if (latStr != null && lonStr != null) {
                        ...
                        val shortName = displayNameStr?.split(',')?.take(2)?.joinToString(",") ?: queryString
