...
java -jar build/libs/pathpress-standalone.jar --start "San Jose, CA" --end "Monterey, CA" --days 1 --pbf data/california-latest.osm.pbf

# Example 2: Multi-day AI trip with Ollama, custom prompt & imperial units
java -jar build/libs/pathpress-standalone.jar --start "San Jose, CA" --end "San Diego, CA" --days 2 --pbf data/california-latest.osm.pbf --llm-provider ollama --prompt "toddler friendly, coastal highway route, prefer scenic beach town or historic village for overnight stay" --units imperial
```

> **Tip**: Run `java -jar build/libs/pathpress-standalone.jar --help` to view all available CLI flags, default values, and usage options.
...
java -jar build/libs/pathpress-standalone.jar --start "Austin, TX" --end "San Antonio, TX" --pbf data/texas-latest.osm.pbf
```
